...
import { fetchUserAvatar } from '../../../store/reducers/ActionCreators';
...
export function Header() {
  ...
  const { avatar, loading } = useAppSelector(state => state.avatar);
  const [size, setSize] = React.useState<Size>({ width: window.innerWidth });


  React.useEffect(() => {
    if (id) dispatch(fetchUserAvatar(id));
      ...
      <div className={['container', styles.header_container].join(' ')}>
        <div className={styles.wrap}>
          <div className={styles.user_avatar}>
            {loading && <AuthSpinner className={styles.spinner} />}
